...
values from the two different distributions can be directly compared.
...
The \textit{empirical rule} is more standardized way of evaluating spread of unknown data. For normally distributed data only:
...
Dropping just a few outliers can dramatically change the skewness. If $|$skewness$|  < 0.8$, then the dataset is acceptable and does not need transformations.
...
Repeated in regression section. Perform non-linear transformations such as $\sqrt{x}$, log(x), log(100-x), $1/x$ to reduce outliers and reduce skewness.
